...
def train_dqn2(loss_fn, optimizer_class, reward_func, n_episodes=1000, max_steps=200):
    env = SimpleMountainCarEnv() 
    alpha = 0.001
    gamma = 0.99
    epsilon = 1.0
    ...
    optimizer = optimizer_class(policy_net.parameters(), lr=alpha)
    criterion = loss_fn()
    ...
    for episode in range(n_episodes):
        ...
        for step in range(max_steps):
            ...
            reward = reward_func(state, action, next_state, reward)
            ...
            if len(replay_buffer.buffer) >= batch_size:
                ...
                with torch.no_grad():
                    max_next_q_values = target_net(next_states).max(1)[0]
                    targets = rewards_batch + gamma * max_next_q_values * (1 - dones)
                loss = criterion(q_values, targets)
...
def original_reward(state, action, next_state, reward):
    return reward

def modified_reward(state, action, next_state, reward):
    return reward + 0.1 * next_state[0]

experiments = [
    {"loss": nn.MSELoss, "optimizer": optim.Adam, "reward": original_reward, "name": "Exp1"},
    {"loss": nn.SmoothL1Loss, "optimizer": optim.Adam, "reward": original_reward, "name": "Exp2"},
    {"loss": nn.MSELoss, "optimizer": optim.RMSprop, "reward": original_reward, "name": "Exp3"},
    {"loss": nn.MSELoss, "optimizer": optim.Adam, "reward": modified_reward, "name": "Exp4"},
    {"loss": nn.SmoothL1Loss, "optimizer": optim.RMSprop, "reward": original_reward, "name": "Exp5"},
    {"loss": nn.SmoothL1Loss, "optimizer": optim.Adam, "reward": modified_reward, "name": "Exp6"},
]
...
for exp in experiments:
    ...
    policy_net, rewards, training_time = train_dqn2(exp["loss"], exp["optimizer"], exp["reward"])
    avg_reward = np.mean(rewards[-50:])  
    avg_steps = len(rewards) / sum(rewards) if sum(rewards)!=0 else 0
    results.append({
        "Experiment": exp["name"],
        "Loss": exp["loss"].__name__,
        "Optimizer": exp["optimizer"].__name__,
        "Reward Function": exp["reward"].__name__,
...
if __name__ == "__main__":
    ...
    dq_steps, dq_success = evaluate_model((q1, q2), "Double Q-learning")
    dqn_steps, dqn_success = evaluate_model(policy_net, "DQN")
    
    # Generate results table
    results = [
        ["Q-learning", q_steps, q_success, len(reward_history), time_q],
        ["Double Q-learning", dq_steps, dq_success, len(double_q_rewards), time_dq],
    ...
    # Reward plot
    plt.subplot(1, 2, 1)
    plt.plot(moving_average(reward_history, 100), label="Q-learning")
    plt.plot(moving_average(double_q_rewards, 100), label="Double Q-learning")
    ...
    dq_steps_history = evaluate_steps_to_goal((q1, q2), "Double Q-learning")
    dqn_steps_history = evaluate_steps_to_goal(policy_net, "DQN")
